...
def select_image(client: ImmichClient, config: Config):
    ...
    return client.get_random(asset_type="IMAGE", tag_id=tag.id)[0]
...
def main(log: str = "INFO") -> None:
  ...
  STARTTLS: %(start_tls)s""",
    ...
    image = select_image(client, config)
...
Response:  %(res)s
            """,
            {
                ...
                "url": e.response.url,
                "req": format_payload(
                    e.request.body if isinstance(e.request, PreparedRequest) else e.request.data
                ),
